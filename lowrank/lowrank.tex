...
is a slight abuse of terminology.  A more standard use of the term ``secular equation'' would involve a determinant, i.e. $\det \mT_n(z) = 0$.  However, for our purposes it will be more convenient to work with $f_n$.
...
By making use of the Sherman-Morrison-Woodbury formula~\cite{golub1996mc}, we can get an expression for $\left(\mS_{n,22} - z \mI_{p-k}\right)^{-1}$.  As a reminder, the Sherman-Morrison-Woodbury formula states that for matrices $\mA, \mB, \mC,$ and $\mD$ with compatible dimensions, the following formula for the inverse holds:
\[
    \left(
        \mA
        +
...
\(
    ...
    \mS_{n,21}
\)
has eight terms, each of which is rather complicated.  For now, we have no choice
but to write them all down, but rest assured we will spend considerable time
...
\end{align}
\end{subequations}
To put it mildly, this is a formidable expression.  Notice, however, that several common subexpressions appear in the terms of $\mT_n(z)$.  We can simplify the formulas for thes subexpressions when $z > \left(1 + \gamma^{-1/2} \right)^2$.
...
\(
    Y_{N,\alpha} = \left( \mE_{n,12} \right)_{i\alpha}
                   \left( \mE_{n,12} \right)_{j\alpha}.
\)
Letting $b_\gamma = \left( 1 + \gamma^{-1/2} \right)^2$, the function
\[
    g(t) 
    = 
    \begin{cases}
