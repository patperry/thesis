...
we can simplify $f_n(\mu, \vx_1)$.  For convenience, we will define
$\mF_n (\mu)$ such that
\[
    \frac{1}{n}
    \mE_{n,12}
    ...
    \mE_{n,12}^\trans
        =
            ...
            \right)
\]
and $\mF_n(\mu) \tod \mF(\mu)$,
with $\mG_n(\mu), \mH_n(\mu), \mK_n, \mL_{n,1},$ and $\mL_{n,2}$ defined similarly.
            ...
            f_{n,1}( \mu, \vx_1 ) 
                ...
                \big( 1 + \gamma^{-1} m(\mu) \big)^{-1} \\
                    ...
                    \Big(
                        ...
                        +
                        \mD \big( \mE_{n,11} - \mG_n(\mu) \big)
                        + 
                        \big( \mE_{n,11} - \mG_n(\mu) \big)^\trans \mD
                    \Big)  \\
                &\quad\quad-
                \mu \, \mH_n(\mu) \bigg] \, \vx_1.
        \end{split}
    \end{equation*}
\end{lemma}
\begin{proof}
...
We will now study the solutions to $f_{n}(\mu, \vx) = 0$.  If $(\mu, \vx)$
is a solution, then so is $(\mu, \alpha \, \vx)$ for any scalar $\alpha$.  We restrict our attention to solutions with $\|\vx \|_2 = 1$.  We also impose a restriction on the sign of $\vx$, namely we require that the component with the largest magnitude is positive, i.e.
\(
    \max_i x_i = \max_i | x_i |.
\)
