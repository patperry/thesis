...
The proof is an adaptation from Stewart \& Sun \cite{stewart1990mpt}[page 167].  It involves Rouch\'e's Theorem, which we state as a lemma.  
\begin{lemma}[Roch\'e's Theorem]\label{L:rouche}
    If $\phi$ and $\eta$ are analytic in a simply connected region
    $\Omega$ and $\mathcal{D} \subset \Omega$ is a neighborhood for which
    \[
        | \eta(z) | < | \phi(z) |, \quad z \in \partial \, \mathcal{D},
    \]
    where $\partial \, \mathcal{D}$ is the boundary of $\mathcal{D}$, then
    $\phi(z)$ and $\phi(z) + \eta(z)$ have the same number of
    zeros in $\mathcal{D}$.
\end{lemma}
