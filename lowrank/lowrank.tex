...
\[
    ...
    \frac{1}{b(\gamma) - \mu}
\]
with $b(\gamma) = \left( 1 + \gamma^{-1/2} \right)^2$ is bounded and 
continuous.  Moreover, since $\lambda_{n,1} \toas b(\gamma)$, with probability one $g(\lambda_{n,\alpha})$ is eventually equal to $W_{N,\alpha}$ for all $\alpha$.  The Wishart LLN (Corollary~\ref{C:wishart-lln}) gives us that
\[
    \frac{1}{N}
...
In fact we can get the second-order behavior of these quantities as well.
