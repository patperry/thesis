...
\noindent
When some $\mu_i$ equals the cutoff value, $\hat k_n$ does not have
a limit, but its behavior is not hard to analyze.
...
\begin{corollary}
	...
	(provided no $\mu_i$ is exactly equal to
		\(
			...
			\sqrt{\frac{2}{\rho}},
		\)
	in which case the limits are ambiguous).
\end{corollary}
	...
	K
		&=
			\frac{3}{3
					 ...
					 \right)}.
\end{align*}
For very large or very small aspect ratios ($\gamma \to 0$ or $\gamma \to \infty$), $\bar \gamma \to \infty$ and $K \to 1$.  In these situations one should leave out almost all of the matrix when performing
bi-cross-validation.
...
We prove Proposition~\ref{P:holdin-svd} by showing that
\(
    \Big(
        \sqrt{\frac{n}{n_1}} \mU_{n,1}
    \Big)
