...
\noindent
This assumption is there so that we can apply the theory in Chapter~\ref{C:svd-behavior} to get at the behavior of the SVD of $\mX_{n,11}$.  It is satisfied, for example, if we are performing rotated cross-validation (see subsection~\ref{SS:rotated-cv}) or if the factors are generated by certain stationary processes.  Proposition~\ref{P:project-kframe} in Appendix~\ref{A:projections} is helpful for verifying Assumption~\ref{A:held-in-orthog}.  It is likely that the theory presented below holds under a weaker condition, but a detailed analysis is beyond the scope of this chapter.
