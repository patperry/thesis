...
\begin{definition}
    ...
    \(
        \mX = \sqrt{n} \, \mU \mD \mV^\trans + \mE,
    \)
    where $\mU^\trans \mU = \mV^\trans \mV = \mI_k$, $\mD$ is a
    $k \times k$ diagonal matrix with positive diagonal entries, and $\mE$
...
\(
    ...
    =
    \diag \left(
        \hd_1, \hd_2, \ldots, \hd_l, 0, 0, \ldots, 0
    \right),
\)
then the SVD truncated to $l$ terms is
\(
    \mhX(l)
    =
    \sqrt{n} \,
...
\begin{definition}
    ...
    \[
        k^\ast
        \define
        \argmin_l \, L \left( \sqrt{n} \mU \mD \mV^\trans\!, \, \mhX(l) \right).
    \]
\end{definition}
