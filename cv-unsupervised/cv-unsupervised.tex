...
We conclude this chapter with an application from Neuroscience. The Neural
...
NPSL has designed and carried out an experiment mean to measure the 
...
holdouts, an EM-like algorithm can usually be applied to the models in many
...
any theoretic results here, only simulations.  A quote from Downton's discussion of Stone's 1974 paper on cross-validation is particularly appropriate here:
\begin{quote}
    ...
    \cite{stone1974cv}.
\end{quote}
In the next chapter, we provide some theoretical justification for 
Gabriel-style cross-validation, but analysis of Wold-style cross-validation is 
